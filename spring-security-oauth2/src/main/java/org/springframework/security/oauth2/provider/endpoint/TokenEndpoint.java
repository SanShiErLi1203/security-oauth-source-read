...
public class TokenEndpoint extends AbstractEndpoint {
	 ...
	 *
	 * 		TokenEnpoint.postAccessToken()为最核心的一部分，代码解析如下：
	 *
	 * @param principal
	 * @param parameters
	 * @return
	...
	Map<String, String> parameters) throws HttpRequestMethodNotSupportedException {
		...
		/** 判断Principal是否为Authentication实例 **/

		/**
		 * Auhentication源码：  --> Authentication继承自 Principal
		...
		/** 调用getClientId(Princial princial)方法获取clientId **/
		String clientId = getClientId(principal);

		/** 根据clientId获取ClientDetails(客户端详情) **/
	...
	protected String getClientId(Principal principal) {
		...
		/** 判断是否已经验证通过，没验证通过则抛出异常 **/
		if (!client.isAuthenticated()) {
			throw new InsufficientAuthenticationException("The client is not authenticated.");
		}
		String clientId = client.getName();

		/** 判断Authentication是否为OAuth2Authentication实例 **/
		if (client instanceof OAuth2Authentication) {
			// Might be a client and user combined authentication
			/** 将Authentication强转为OAuth2Authentication，调用其getOAuth2Request().getClientId()方法，获取clientId **/
